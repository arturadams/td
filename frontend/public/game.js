...
class TowerDefenseGame {
    ...
    update(){
        ...
        if(this.isWaveActive&&this.waveEnemyCount===0){this.isWaveActive=false;if(this.currentWave>=this.config.waves.length){this.showEndScreen(true);}else{const btn=document.getElementById('start-wave-btn');btn.disabled=false;btn.classList.remove('opacity-50','cursor-not-allowed');}}
    }
    ...
    checkEnemyDeath(enemy){if(enemy.health<=0){this.money+=enemy.money;this.updateStats();const idx=this.enemies.indexOf(enemy);if(idx>-1){this.enemies.splice(idx,1);this.waveEnemyCount--;}}}
